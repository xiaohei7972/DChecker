...
object HomeChineseText {
    private val exact = mapOf(
        ...
        "Matched" to "匹配",
        "Mismatch" to "不一致",
        "Runtime and attested patch levels line up locally." to "运行时与证明中的补丁级别在本机一致。",
        "Patch levels drift slightly but stay within a short window." to "补丁级别存在轻微偏差，但仍处于较短时间窗口内。",
        "Runtime and attested patch levels drift by more than three months." to "运行时与证明中的补丁级别相差超过三个月。",
        "Patch comparison was unavailable." to "补丁级别比较不可用。",
        "locked" to "已锁定",
        "unlocked" to "已解锁",
        "lock unknown" to "锁定状态未知",
        "No user auth required" to "无需用户认证",
        "Raw app attestation present" to "存在原始应用证明",
        "Observed • local chain failed" to "已观察 • 本地证书链失败",
        "Observed • local trust needs review" to "已观察 • 本地信任需要核查",
        "Provisioned" to "已配置",
        "Review provisioning" to "检查配置状态",
        "Online" to "在线",
        "Built-in snapshot" to "内置快照",
        "Delete ok • fresh material" to "删除正常 • 已生成新材料",
        "Delete/regenerate contradiction" to "删除/重新生成结果矛盾",
        "Round-trip failed" to "往返测试失败",
        "Auth failed" to "认证失败",
        "Round-trip ok" to "往返测试正常",
        "Round-trip ok • software-backed" to "往返测试正常 • 软件支持",
        "Measurement unavailable" to "测量不可用",
        "Ratio skipped" to "比率计算已跳过",
        "Positive" to "阳性",
        "Not positive" to "未发现阳性",
        "Register timer" to "寄存器计时器",
        "Fallback timer" to "备用计时器",
        "Timer source unspecified" to "计时器来源未指定",
        "Marker preserved" to "标记已保留",
        "Marker replaced" to "标记已替换",
        "TEE Simulator generate-mode fingerprint probe unavailable." to "TEE 模拟器 generate 模式指纹探针不可用。",
        "Java-style reply" to "Java 风格回复",
        "Native-style reply" to "原生风格回复",
        "Unexpected reply" to "异常回复",
        "Security level exposed" to "已暴露安全等级",
        "No security level exposed" to "未暴露安全等级",
        "Metadata security level exposed" to "元数据中暴露安全等级",
        "No metadata security level exposed" to "元数据中未暴露安全等级",
        "KEY_ID normalized" to "KEY_ID 已规范化",
        "Descriptor mismatch" to "描述符不一致",
        "System fields present" to "系统字段存在",
        "System fields missing" to "系统字段缺失",
        "createOperation failed" to "createOperation 失败",
        "updateAad mismatch" to "updateAad 不一致",
        "Oversized update accepted" to "接受了超大 update",
        "Abort left operation alive" to "Abort 后操作仍存活",
        "Compatibility params required" to "需要兼容参数",
        "Native-style errors" to "原生风格错误",
        "Key-not-found style failure" to "类似“密钥不存在”的失败",
        "No anomaly" to "未发现异常",
        "Unexpected failure" to "异常失败",
        "No comparable IDs exposed" to "未暴露可比较的设备 ID",
        "Available fields aligned" to "可用字段一致",
        "Strong biometric unavailable" to "强生物识别不可用",
        "User-auth key path available" to "用户认证密钥路径可用",
        "Created but getKey() returned null" to "已创建，但 getKey() 返回 null",
        "User-auth key path failed" to "用户认证密钥路径失败",
        "Hook bootstrap failed" to "Hook 引导失败",
        "Leaf SPKI matched issuer SPKI" to "叶证书 SPKI 与签发者 SPKI 一致",
        "Repeated active probe failed" to "重复活动探针失败",
        "deleteEntry left alias present" to "deleteEntry 后别名仍存在",
        "Java chain unavailable" to "Java 证书链不可用",
        "Binder chain unavailable" to "Binder 证书链不可用",
        "generateKey leaf differed from getKeyEntry" to "generateKey 叶证书与 getKeyEntry 不一致",
        "generateKey chain differed from getKeyEntry" to "generateKey 证书链与 getKeyEntry 不一致",
        "Java and binder chains aligned" to "Java 与 Binder 证书链一致",
        "Leaf matched but chain diverged" to "叶证书一致，但证书链不同",
        "Leaf and chain diverged" to "叶证书与证书链均不同",
        "Hook installed" to "Hook 已安装",
        "Hook unavailable" to "Hook 不可用",
        "Legacy path not observed" to "未观察到旧版路径",
        "Legacy path aligned" to "旧版路径一致",
        "Legacy path captured but diverged" to "已捕获旧版路径，但结果不一致",
        "Legacy path unavailable" to "旧版路径不可用",
        "Leaf differed between generateKey and getKeyEntry" to "generateKey 与 getKeyEntry 的叶证书不同",
        "Chain differed between generateKey and getKeyEntry" to "generateKey 与 getKeyEntry 的证书链不同",
        "generateKey/getKeyEntry aligned" to "generateKey/getKeyEntry 一致",
        "Capture unavailable" to "捕获不可用",
        "Not advertised" to "未声明支持",
        "Available" to "可用",
        "Not confirmed" to "未确认",
        "BadParcelable-style crash" to "BadParcelable 类崩溃",
        "containsAlias/listEntries mismatch" to "containsAlias/listEntries 不一致",
        "containsAlias and aliases aligned" to "containsAlias 与 aliases 一致",
        "Cursor echoed in page" to "页面中回显了游标",
        "Expected next alias missing" to "缺少预期的下一个别名",
        "Cursor semantics aligned" to "游标语义一致",
        "Primary DER hit" to "主 DER 命中",
        "No local process-side anomaly" to "本地进程侧未发现异常",
        "Short drift" to "短期偏差",
        "Wide drift" to "较大偏差",
        "Revoked" to "已吊销",
        "Mass abuse" to "大规模滥用",
        "Built-in" to "内置",
        "Offline" to "离线",
        "Primary DER" to "主 DER",
        "Secondary DER" to "次级 DER",
        "Tracing" to "跟踪",
        "State only" to "仅状态",
        "Possible cause: vendor binder/libc compatibility differences. No stronger hook fingerprint was found." to "可能原因：厂商 Binder/libc 兼容性差异。未发现更强的 Hook 指纹。",
        "Map hit" to "内存映射命中",
    ...
    private val countPatterns = listOf(
        Regex("""^(\d+) mismatch\(es\)$""") to { m: MatchResult -> "${m.groupValues[1]} 项不一致" },
        Regex("""^(\d+) comparable field\(s\) not exposed$""") to { m: MatchResult -> "${m.groupValues[1]} 个可比较字段未暴露" },
        Regex("""^(\d+) suspicious mapping\(s\)$""") to { m: MatchResult -> "${m.groupValues[1]} 个可疑内存映射" },
        Regex("""^(\d+) package\(s\) • (\d+) signer digest\(s\)$""") to { m: MatchResult ->
            "${m.groupValues[1]} 个应用包 • ${m.groupValues[2]} 个签名摘要"
        },
    ...
    fun translate(text: String): String {
        ...
        Regex("""^Provisioned • (.+)d leaf$""").matchEntire(text)?.let {
            return "已配置 • 叶证书有效期 ${it.groupValues[1]} 天"
        }
        Regex("""^Error (.+)$""").matchEntire(text)?.let {
            return "错误 ${it.groupValues[1]}"
        }
        Regex("""^(.+) skipped$""").matchEntire(text)?.let {
            val label = translate(it.groupValues[1])
            if (label != it.groupValues[1]) return "$label 已跳过"
        }
